...
const RecursiveApplicationCard: React.FC<ApplicationCardProps> = ({ 
  ...
  return (
    <Card 
      ref={cardRef}
      className={cn("lg:flex gap-2 lg:gap-4 items-center p-3 lg:p-4 duration-300 transform max-w-full border rounded-3xl transition-all",hoveredCardId === id ? "ring-2 ring-primary/20 shadow-[0_8px_30px_rgb(0,0,0,0.12)] scale-[1.02] z-10 backdrop-blur" : ""
      )}
      data-id={id}
    >
       <CardContent className="p-2 flex flex-col gap-2">
          ...
          <p className="text-xs sm:text-sm text-gray-500">
            {currentApp.users.length} {hoveredCardId === id && cardTitle === 'Overlapping spend' && `Overlapping` } users
          </p>
       </CardContent>

      {/* Recursive child - only render if we haven't reached maxDepth */}
      {depth < maxDepth && (
        <div 
...
const OverlappingApplications: React.FC = () => {
  ...
  return (
    <div className="flex flex-col p-2 sm:p-4 w-full max-w-full bg-white rounded-2xl shadow-lg">
      {/* tabs */}
      <div className="flex justify-between items-center overflow-x-auto gap-4 sm:gap-10 mb-4 whitespace-nowrap">
        {apps.map((app, idx) => 
          <div key={idx} className='flex justify-between items-center rounded-md border-2 px-2 sm:px-4 py-1 sm:py-2 flex-shrink-0 min-w-[150px] sm:min-w-[200px]'>
